// ============================================================
// function.js — versão otimizada (latência baixa / hedge / cache)
// ============================================================
...
// ------------------------------------------------------------
// Constantes de API e utilidades de rede
// ------------------------------------------------------------
const API_BASE = 'https://proxy-g.vercel.app/api/proxy';
const TIMEOUT_MS_PRIMARY = 7000;    // primária: 7s
const HEDGE_DELAY_MS = 250;         // solta a hedge quase junto
const TIMEOUT_MS_HEDGE = 9000;      // hedge: 9s
const CACHE_TTL_MS = 1000 * 60 * 30; // 30 min

// Validação de CPF (evita chamadas inúteis)
function isValidCPF(str) {
  const cpf = String(str).replace(/\D/g, '');
  if (!cpf || cpf.length !== 11 || /^(\d)\1+$/.test(cpf)) return false;
  let soma = 0, resto;
  for (let i = 1; i <= 9; i++) soma += parseInt(cpf.substring(i - 1, i), 10) * (11 - i);
  resto = (soma * 10) % 11; if (resto === 10 || resto === 11) resto = 0; if (resto !== parseInt(cpf.substring(9, 10), 10)) return false;
  soma = 0;
  for (let i = 1; i <= 10; i++) soma += parseInt(cpf.substring(i - 1, i), 10) * (12 - i);
  resto = (soma * 10) % 11; if (resto === 10 || resto === 11) resto = 0; return resto === parseInt(cpf.substring(10, 11), 10);
}

// Abort com timeout
function fetchWithTimeout(resource, options = {}, ms = 5000) {
...
// Faz parse seguro de JSON mesmo que venha como text/plain ou com “ruído”
async function parseJsonResponse(r) {
  ...
  // tenta extrair um bloco JSON final (caso tenha logs/lixo antes)
...
// ------------------------------------------------------------
// Cache em localStorage com TTL (stale-while-revalidate)
// ------------------------------------------------------------
const LS_KEY = 'cpf_cache_v1';
...
function getCachedCPF(cpf) {
  const store = readCache();
  const item = store[cpf];
  if (!item) return null;
  const isFresh = Date.now() - item.ts < CACHE_TTL_MS;
  return { data: item.data, isFresh };
}
function setCachedCPF(cpf, data) {
  const store = readCache();
  store[cpf] = { data, ts: Date.now() };
  writeCache(store);
}

// ------------------------------------------------------------
// Estratégia de “hedge requests” + fallback via AllOrigins
// - Dispara requisição direta
// - Após pequeno delay, dispara via proxy AllOrigins para a MESMA URL
// - Usa a que responder primeiro (race manual)
// ------------------------------------------------------------
function buildApiUrl(cpf) {
  const u = new URL(API_BASE);
  u.searchParams.set('cpf', cpf);
  return u.toString();
}
...
// “Race” manual com resolve-once e coleta de erros (sem Promise.any)
async function hedgeFetchJson(apiUrl) {
  let resolved = false;
  let errors = [];

  return new Promise((outerResolve, outerReject) => {
    const finishOk = (data) => { if (!resolved) { resolved = true; outerResolve(data); } };
    const finishErr = (err) => {
      errors.push(err);
      if (errors.length >= 2 && !resolved) outerReject(errors[errors.length - 1]);
    };

    // 1) Primária direta
    fetchWithTimeout(apiUrl, { headers: { 'Accept': 'application/json, text/plain, */*' } }, TIMEOUT_MS_PRIMARY)
      .then(parseJsonResponse)
      .then(finishOk)
      .catch(finishErr);

    // 2) Hedge (via AllOrigins), disparada após pequeno atraso
    setTimeout(() => {
      const proxyUrl = buildAllOriginsUrl(apiUrl);
      fetchWithTimeout(proxyUrl, { headers: { 'Accept': 'application/json, text/plain, */*' } }, TIMEOUT_MS_HEDGE)
        .then(parseJsonResponse)
        .then(finishOk)
        .catch(finishErr);
    }, HEDGE_DELAY_MS);
...
// Consulta com cache SWR: tenta cache; retorna imediato se houver,
// enquanto revalida em background
async function fetchCpfData(cpf, { allowStale = true } = {}) {
  const cached = getCachedCPF(cpf);

  if (allowStale && cached?.data) {
    // revalida em background
    hedgeFetchJson(buildApiUrl(cpf))
      .then((fresh) => { if (fresh?.dadosBasicos) setCachedCPF(cpf, fresh); })
      .catch(() => {});
    return { data: cached.data, fromCache: true };
  }

  // Rede (hedge)
  const fresh = await hedgeFetchJson(buildApiUrl(cpf));
  if (!fresh || !fresh.dadosBasicos) throw new Error('Resposta inesperada da API');
  setCachedCPF(cpf, fresh);
...
async function prefetchComprovante() {
  ...
  // tentativa direta; se der erro/CORS, tenta via AllOrigins
  let json;
  try {
    const r = await fetch(endpoint, { headers: { 'Accept': 'application/json, text/plain, */*' } });
    json = await parseJsonResponse(r);
  } catch (e1) {
    const proxyUrl = buildAllOriginsUrl(endpoint);
    const r2 = await fetch(proxyUrl, { headers: { 'Accept': 'application/json, text/plain, */*' } });
    json = await parseJsonResponse(r2);
  }
...
// ============================================================
// Fluxo principal (form + timer) — com pré-consulta e SWR
// ============================================================
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Pré-consulta com debounce (aquecimento de cache)
  let prefetchTid = null;
  cpfInput.addEventListener('keyup', () => {
    clearTimeout(prefetchTid);
    prefetchTid = setTimeout(async () => {
      const cpf = cpfInput.value.replace(/\D/g, '');
      if (cpf.length === 11 && isValidCPF(cpf)) {
        // não bloqueia a UI
        fetchCpfData(cpf, { allowStale: false }).catch(() => {});
      }
    }, 400);
  });
  ...
  form.addEventListener('submit', async function (e) {
    ...
    try {
      // 1) tenta cache imediato (SWR). Se vier cache, já avança de etapa.
      const { data, fromCache } = await fetchCpfData(cpf, { allowStale: true });
      ...
      // 2) Preenche storage e UI
      hydrateUserData(data, cpf);

      // 3) Avança logo a UI (percepção de velocidade)
      step1.classList.add('hidden');
      step2.classList.remove('hidden');
      updateStep2Bindings();

      // 4) Se veio do cache, revalida em background e atualiza se houver mudança
      if (fromCache) {
        hedgeFetchJson(buildApiUrl(cpf))
          .then((fresh) => {
            if (fresh?.dadosBasicos) {
              setCachedCPF(cpf, fresh);
              hydrateUserData(fresh, cpf);
              updateStep2Bindings();
            }
          })
          .catch(() => {});
      ...
      console.error('[CPF] Falha na consulta:', error && (error.stack || error.message || error));
      alert('Erro ao consultar o CPF. Tente novamente em instantes.');
    } finally {
  ...
  function hydrateUserData(userData, fallbackCpf) {
    localStorage.setItem('dadosBasicos', JSON.stringify(userData));
    localStorage.setItem('cpf', String(userData?.dadosBasicos?.cpf || fallbackCpf || '').replace(/\D/g, ''));
  ...
  function updateStep2Bindings() {
    const nameValue = localStorage.getItem('name') || '';
    const cpfValue = localStorage.getItem('cpf') || '';
    const el1 = document.getElementById('nameUser');
    const el2 = document.getElementById('nameUser2');
    const elCpf = document.getElementById('cpfUser');
    if (el1) el1.textContent = nameValue;
    if (el2) el2.textContent = nameValue;
    if (elCpf) elCpf.textContent = cpfValue;
  }
